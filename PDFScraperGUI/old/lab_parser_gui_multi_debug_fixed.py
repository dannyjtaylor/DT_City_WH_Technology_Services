...
def parse_ocr(pdf_path, temp_dir):
    ...
    for i, img in enumerate(images):
        ...
        img.save(img_path)
        print(f"[DEBUG] Saved image for OCR: {img_path}")
...
def extract_parameters(text):
    ...
    print(f"[DEBUG] Found {len(parameters)} parameter entries.")
    return parameters
...
def convert_to_csv(pdf_path, csv_path):
    ...
    try:
        ...
        print(f"[INFO] Final extracted text from document:\n{text}\n{'='*80}")
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for param in param_entries:
                ...
                print(f"[DEBUG] Writing CSV row: {row}")
                writer.writerow(row)
        os.startfile(csv_path)
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)
        print(f"[INFO] Cleaned up temporary folder: {temp_dir}")
