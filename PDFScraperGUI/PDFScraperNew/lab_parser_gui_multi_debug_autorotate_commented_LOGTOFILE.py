...
def extract_text(pdf_path):
    try:
        ...
        log(f"[ERROR] Failed to extract text using PyPDF2: {e}")
        return None

def parse_ocr(pdf_path, temp_dir):
    log("[INFO] Falling back to OCR...")
    debug_img_dir = os.path.join(os.getcwd(), "debug_images")
    ...
    for i, img in enumerate(images):
        try:
            ...
            if rotate_angle != 0:
                ...
                log(f"[INFO] Rotated page {i+1} by {rotate_angle}° for OCR alignment.")
        except Exception as e:
            log(f"[WARNING] Orientation detection failed on page {i+1}: {e}")

        img_path = os.path.join(debug_img_dir, f"page_{i+1}.png")
        img.save(img_path)
        log(f"[DEBUG] Saved page image for OCR: {img_path}")

        ocr_text = pytesseract.image_to_string(img)
        log(f"[DEBUG] OCR output from page {i+1}:\n{ocr_text}\n{'-'*60}")
        text += ocr_text + "\n"
    return text
...
def extract_parameters(text):
    ...
    log(f"[DEBUG] Found {len(parameters)} parameters.")
    return parameters
...
def convert_to_csv(pdf_path, csv_path):
    ...
    try:
        ...
        log(f"[INFO] Final text used for parsing (first 1000 chars):\n{text[:1000]}...\n{'='*80}")
        meta = extract_metadata(text)
        log(f"[DEBUG] Metadata extracted: {meta}")
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for param in param_entries:
                ...
                log(f"[DEBUG] Writing row: {row}")
                writer.writerow(row)

        log(f"[SUCCESS] CSV saved to: {csv_path}")
        os.startfile(csv_path)
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)
        log(f"[INFO] Cleaned up temporary directory: {temp_dir}")
...
def main():
    ...
    if not save_path:
        return
    log(f"[START] Converting file: {pdf_path}")
    convert_to_csv(pdf_path, save_path)
    log(f"[DONE] Conversion complete.")
